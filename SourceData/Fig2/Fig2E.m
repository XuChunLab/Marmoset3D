...
% scatter3(data_3d(spikeatframe,1),data_3d(spikeatframe,2),data_3d(spikeatframe,3),10,"red","filled")
% view(3)
% daspect([1 1 1]);
...
%*****2 half***************************************************************
%figure('Position',[100 520 560 420])
subplot(142)
clear data_3d spikeatframe
data_3d=reserveddata_3d;
spikeatframe=reservedspikeatframe;
visitplace=zeros(20,20,10);
for i=1:floor(size(data_3d,1)/2)
    temp=[];
    if ~isnan(data_3d(i,1))
        temp=ceil(data_3d(i,:)/75);
        visitplace(temp(1),temp(2),temp(3))=visitplace(temp(1),temp(2),temp(3))+1;
    end
end
frplace=zeros(20,20,10);
data_3d=data_3d(spikeatframe,:);
for i=1:floor(size(data_3d,1)/2)
    temp=[];
    if ~isnan(data_3d(i,1))
        temp=ceil(data_3d(i,:)/75);
        frplace(temp(1),temp(2),temp(3))=frplace(temp(1),temp(2),temp(3))+1;
    end
end
data_3d=[];
clear data_3d
reservevisitplace=visitplace;
visitplace(find(visitplace<ceil(visitinglimitation/2)))=nan; 
fr3Dmap=frplace./(visitplace*0.05);
...
% 230529 new version!!!!
    for i=1:20
        for j=1:20
            for k=1:10
                temp=2;
                while isnan(fr3Dmap(i,j,k))
                    a=(i-temp:1:i+temp);a(a<1)=[];a(a>20)=[];
                    b=(j-temp:1:j+temp);b(b<1)=[];b(b>20)=[];
                    c=(k-temp:1:k+temp);c(c<1)=[];c(c>10)=[];
                    fr3Dmap(i,j,k)=nanmean(nanmean(nanmean(fr3Dmap(a,b,c),3)));
                    temp=temp+1;
                end
                temp=2;
            end
...
fr3Dmap = smooth3(fr3Dmap,'gaussian',[5 5 5],1);% 230529 new version!!!!

fr3Dmap(find(reservevisitplace<ceil(visitinglimitation/2)))=nan;
...
view(3)
daspect([1 1 1]); 
the3dmap_1sthalf=temp;
set(gcf,'Color',[1 1 1])
set(gca,'Color',[0.9 0.9 0.9]) 

%figure('Position',[660 520 560 420])
subplot(143)
clear data_3d spikeatframe
data_3d=reserveddata_3d;
spikeatframe=reservedspikeatframe;
visitplace=zeros(20,20,10);
for i=floor(size(data_3d,1)/2):size(data_3d,1)
    temp=[];
    if ~isnan(data_3d(i,1))
...
%         scatter3(center(1)/0.075,center(2)/0.075,center(3)/0.075,60,'MarkerFaceColor',[1,0.375,0],'MarkerEdgeColor','none');
%         hold on
%     end
% end 
% xlim([0 20]);ylim([0 20]);zlim([0 10])
